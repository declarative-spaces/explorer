export const VIEWPORT_WIDTH = 9;
export const VIEWPORT_HEIGHT = 16;

export function parseDslNumber(raw) {
...
export function parseDslLine(line, idx) {
...
export function resolveCollisions(objects) {
  const accepted = [];
  const rejected = [];
...
export function clipToViewport(object, cameraX) {
...
export function compilePrompt(visibleObjects, cameraX) {
  const sceneHeader = [
  ...
  const objectLines = visibleObjects.map((o, i) => (
...
export function renderPlaceholderSvg(cameraX) {
