...
const ROOM_DEPTH = 16;
const WALL_THICKNESS_UNITS = 0.3;
...
const isCutoutObject = (o: Pick<DslObject, 'z'>): boolean => o.z.offset === 0 && o.z.size === 0;
...
function DefaultRoom({ cutouts }: { cutouts: DslObject[] }) {
  const wallWidth = ROOM_WIDTH * UNIT_SCALE;
  const wallHeight = ROOM_HEIGHT * UNIT_SCALE;
  const wallThickness = WALL_THICKNESS_UNITS * UNIT_SCALE;

  return (
    <>
      <mesh receiveShadow rotation-x={-Math.PI / 2} position={[wallWidth / 2, 0, ROOM_DEPTH * UNIT_SCALE / 2]}>
        <planeGeometry args={[wallWidth, ROOM_DEPTH * UNIT_SCALE]} />
        <meshStandardMaterial color="#d7d2cb" roughness={0.85} />
      </mesh>

      <mesh receiveShadow castShadow position={[wallWidth / 2, wallHeight / 2, 0]}>
        <Geometry>
          <Base>
            <boxGeometry args={[wallWidth, wallHeight, wallThickness]} />
          </Base>
          {cutouts.map((o) => {
            const cutoutWidth = o.x.size * UNIT_SCALE;
            const cutoutHeight = o.y.size * UNIT_SCALE;
            const cutoutCenterX = (o.x.offset + o.x.size / 2) * UNIT_SCALE;
            const cutoutCenterY = (o.y.offset + o.y.size / 2) * UNIT_SCALE;

            return (
              <Subtraction key={o.id} position={[cutoutCenterX, cutoutCenterY, 0]}>
                <boxGeometry args={[cutoutWidth, cutoutHeight, wallThickness * 3]} />
              </Subtraction>
            );
          })}
        </Geometry>
...
function DslObjects({ objects }: { objects: DslObject[] }) {
  const acceptedObjects = objects.filter((o) => o.status === 'accepted' && !isCutoutObject(o));
...
export default function App() {
  ...
  const cutouts = objects.filter((o) => o.status === 'accepted' && isCutoutObject(o));
  ...
  return (
    <div className="app-root">
      <Canvas shadows="basic" camera={{ position: [5, 4, 10], fov: 40 }}>
        ...
        <DefaultRoom cutouts={cutouts} />
        <DslObjects objects={objects} />
